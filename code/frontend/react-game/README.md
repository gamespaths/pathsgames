...
│   ├── mapGraph.js     # (v0.28.5) buildMapGraph(info, matchLocations), edgeVisibility(edge, isVisited) — pure graph layout for the interactive world map
│   └── gameResults.js  # (v0.35.5) grantedItemUuids, itemRowForUuid, itemCardForUuid, lastEffectCard — re-exported from GameBook.jsx for the existing test suites
├── components/
│   ├── layout/     # Navbar, Footer, GameCard, GameCardCreditsBar (credits row), GameCardInfoButton (legacy, no longer used by GameCard), Card.jsx (onForward prop, v0.28.5), Map.jsx (MapPage — interactive world map, v0.28.5)
...
│   └── gameplay/   # GameBook (composition only, ~170 lines, v0.35.5), EndGameBook, GameBookMobile, ClockWidget
│       ├── PageLeft.jsx       # left page: choice-event > edge state (coma/sadness) > backpack > map > left preview > current location > story card
│       ├── PageRight.jsx      # right page: preview > choice-event > wake-up list > selected map node > backpack > stats list > PageRightMain; RightPreview switches on `kind`
...
│       ├── cards/             # ActionCard, ChoiceCard, ComaCard, SadnessCard, WeatherCard, ItemCard(s), MovementCard, PlayerCards, GoToSleepCard, LocationCard, PlayerStats, MapCard, AutomaticEvents, PendingChoicesList, InformationCard, EndGameCard, CloseGameCard
...
4. **Game** (`/play/:storyId`) — book layout, composed by `GameBook.jsx` (v0.35.5: pure composition, ~170 lines) out of `PageLeft`/`PageRight`/`PageRightMain`/`PageRightInfo` and three hooks (`useMatchChrome` for clock/weather/locations, `useBookView` for the view-state reducer, `useGameplayResults` for reload + API narration). `PageLeft` picks, in priority order, an open choice-event, an edge state (coma/sadness), the backpack, the map, a left-side preview, the current location card, or the story card. `PageRight` picks a preview, an open choice-event, the wake-up (counter-zero) list, a selected map node, the backpack, the stats list, or `PageRightMain` (the board: stat cards, `ComaCard`, `GoToSleepCard`, one `MovementCard` per neighbor, one `ActionCard`/`EndGameCard` per action). Card previews go through a single `onPreview({ card, type, lockedReason, stats, modal, props, side })` object call (gameplay-only; unrelated to the 4-argument `onPreview` used by `start-book`/`start-match`/`MatchCard`/`GuestUserModal`) — a null `card` closes whatever preview is open. Click card → detail modal with move/execute button. Navbar + Footer always present. **Interactive world map** (v0.28.5): opened from the `MapCard` in the stats card list, renders `MapPage` (`components/layout/Map.jsx`) as the left page — a pan/zoom SVG graph (built by `utils/mapGraph.js` from `GET /api/match/{uuid}/locations` + match-info) with location-photo nodes, a fog-of-war "?" for unvisited locations, a "you are here" marker, and gold two-way/one-way/exit arrows. The right page shows the selected node's `LocationCard`; a new forward arrow (`onForward` on `Card.jsx`, `onEnterLocation` on `LocationCard.jsx`) returns to normal gameplay when the selected node is the character's current location. Available on desktop and mobile. The "go to sleep" shortcut on the stat card (`GoToSleepCard`, `autoPreview` prop) opens its own reading page directly instead of a `document.querySelector` click on the Sleep button — the old lookup keyed off the English aria-label and silently no-op'd under Italian.
4. **i18n** — IT / EN via language switcher in Navbar. The selected language is persisted to `localStorage` under the key `pathsgames.lang` by `src/i18n/context.jsx` (`LanguageProvider`). On mount the initial language is resolved in order: (1) user's saved choice in `localStorage['pathsgames.lang']`; (2) browser language via `navigator.language` (e.g. `it-IT` → `it`), only if supported (`en`/`it`); (3) fallback `'en'`. An explicit user choice always wins over the browser language. All labels are in `src/i18n/en.json` + `it.json`. The `book.stats.*` namespace holds labels for every bonus/stat field shown in the preview panel (`lifeMax`, `energyMax`, `sadMax`, `dexterityStart`/`Base`, `intelligenceStart`/`Base`, `constitutionStart`/`Base`, `weightMax`, `costPositive`, `costNegative`, `expCost`, `maxWeight`, `minCharacter`, `maxCharacter`, `costHelpComa`, `expCostBase`, `maxStatValue`, `numberMaxFreeAction`, and the seven trait stat-delta keys `life`, `energy`, `sad`, `dexterity`, `intelligence`, `constitution`, `weight` added v0.19.6) plus `book.stats.title` ("Bonuses"). The sub-object `book.stats.totals` holds short labels for the eight ConfigView category pills: `life`, `energy`, `sad`, `dexterity`, `intelligence`, `constitution`, `weight`, `exp`. The `card.*` namespace holds labels used by `GameCard`: `card.info` ("Info" / "Info") for the circular info button and `card.viewOriginal` ("View original" / "Vedi originale") for the detail modal link. The `modals.guestUser.*` namespace (added v0.19.8) holds `title`, `anonymous`, `uuidLabel`, and `body` (HTML) for the `GuestUserModal`. The `startMatch.*` namespace (added v0.19.10) holds labels for the StartMatchPage countdown and status messages; also added missing Italian `book.singleDesc` / `book.guestDesc` keys.
...
- **Document Version**: 0.37.3
    ...
    | 0.19.3 | GameCard full redesign: unified gc-* layout (gc-title, gc-title__text, gc-title__btn, gc-img, gc-placeholder, gc-footer, gc-footer__btn, gc-footer__btn--selected, gc-footer__cards-buttons, gc-footer__label) replaces card-big/config-card-cover inner layout; golden title bar with magnifier, full-width image, parchment-light footer with action button only | May 18, 2026 |
