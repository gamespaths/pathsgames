...
4. **Game** (`/play/:storyId`) — book layout. Left: current location card. Right: player stats (Life/Energy/Sadness/XP/Food/Magic/Coins/Weight) + neighbor locations row + actions row. Click card → detail modal with move/execute button. Navbar + Footer always present. **Interactive world map** (v0.28.5): opened from the `MapCard` in the stats card list, renders `MapPage` (`components/layout/Map.jsx`) as the left page — a pan/zoom SVG graph (built by `utils/mapGraph.js` from `GET /api/match/{uuid}/locations` + match-info) with location-photo nodes, a fog-of-war "?" for unvisited locations, a "you are here" marker, and gold two-way/one-way/exit arrows. The right page shows the selected node's `LocationCard`; a new forward arrow (`onForward` on `Card.jsx`, `onEnterLocation` on `LocationCard.jsx`) returns to normal gameplay when the selected node is the character's current location. Available on desktop and mobile.
