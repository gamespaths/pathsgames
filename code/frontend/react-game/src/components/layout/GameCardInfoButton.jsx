 ...
 *
 * Props:
 *   card           — card object: urlImage (or url), copyrightText, linkCopyright, title/name
 *   story          — optional story for a "Story" credit row (story.card.urlImage, story.author)
 ...
 *
 * Returns null if card has neither urlImage/url nor copyrightText.
 */
export default function GameCardInfoButton({
  ...
  const urlImage = card?.urlImage ?? null

  if (!urlImage && !card?.copyrightText) return null

  const credits = [
    story?.card?.urlImage ? {
      label: 'Story',
      urlImage: story.card.urlImage,
      name: story.author,
    ...
    } : null,
    {
      label: 'Image',
      urlImage: urlImage,
      icon: urlImage ? undefined : 'fas fa-image',
