#!/usr/bin/env bash
# redeploy.sh — Roll the latest TEST image onto the already-running EC2 instance.
#
# Connects over SSH, (re)writes /opt/pathsgames/backend.env, then:
...
#
# It is self-contained: it works even on instances created by an older start.sh
# (it ships the env-file itself; secrets go over SSH stdin, never on the cmdline).
#
# Reads config from the shared .env and connection info from .state.
# Build & push a fresh image first with ../build_docker_test_and_push.sh.
#
# Usage:
...
# ── Config: prefer .state values, else the root .env *_TEST(_EC2) vars ────────
...
# Deployment environment shown by /api/echo/status (same value sam deploy passes
# as its `Environment` parameter, reused from AWS_ENVIRONMENT_NAME_TEST).
...
# Save the IP from .state first (live lookup below may overwrite it).
STATE_IP="${PUBLIC_IP:-}"
...
# ── 1. Ship the env-file (secrets via stdin, not on the command line) ───────
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  Backend redeployed with the latest test image           ║"
echo "╠══════════════════════════════════════════════════════════╣"
