#!/usr/bin/env bash
# start.sh — Launch an EC2 instance running Paths Games Java backend with plain Docker
...
#                  (Java Spring Boot, PULLED from Docker Hub — built locally by
#                   ../build_docker_test_and_push.sh, NOT built on the instance)
#
# What it does:
...
#        - Writes /opt/pathsgames/backend.env (DB creds, JWT, ports)
...
# ── Config — bound from the ROOT .env (shared *_TEST + EC2-specific *_TEST_EC2) ─
...
# Build & push it with code/scripts/test/build_docker_test_and_push.sh
...
# Deployment environment reported by /api/echo/status — same value sam deploy
# passes as its `Environment` parameter (reused from AWS_ENVIRONMENT_NAME_TEST).
...
ROUTE53_RECORD_NAME="${ROUTE53_RECORD_NAME_TEST_EC2:-api-test-server2.paths.games}"
...
# When enabled, ROUTE53_RECORD_NAME becomes a Route53 ALIAS to the distribution
# and the distribution origin is the EC2 public DNS name (no extra record, no loop).
# Requires Route53 (AWS_DOMAIN_HOSTED_ZONE_TEST) + an ACM cert in us-east-1.
ENABLE_CLOUDFRONT="${ENABLE_CLOUDFRONT_TEST_EC2:-false}"
ACM_CERT_ARN="${CLOUDFRONT_DOMAIN_CERTIFICATE_ARN_TEST_EC2:-}"       # MUST be a us-east-1 cert covering ROUTE53_RECORD_NAME
CLOUDFRONT_PRICE_CLASS="${CLOUDFRONT_PRICE_CLASS_TEST_EC2:-PriceClass_100}"
...
INSTANCE_NAME=${INSTANCE_NAME_TEST_EC2:-"api-test-server2"}
SG_NAME="${INSTANCE_NAME}-sg"

# Tags applied to every taggable resource we create (SG, instance, volume).
# env is configurable via .env (ENV_TAG); project is fixed to PathsGames.
ENV_TAG="${ENV_TAG:-test}"
PROJECT_TAG="PathsGames"
# create-tags syntax (space-separated Key=..,Value=..)
COMMON_TAGS="Key=env,Value=$ENV_TAG Key=createdBy,Value=SH Key=project,Value=$PROJECT_TAG"
# run-instances --tag-specifications syntax (a per-resource Name is prepended)
...
# Image updates on a running instance are handled by redeploy.sh, not start.sh.
# (Skipped for --dry-run so the user-data preview still works.)
INSTANCE_ID="None"
if [ "${1:-}" != "--dry-run" ]; then
...
if [ "$SG_ID" = "None" ] || [ -z "$SG_ID" ]; then
    ...
    SG_ID="$(aws ec2 create-security-group \
        --region "$AWS_REGION" \
        --group-name "$SG_NAME" \
        --description "Paths Games $INSTANCE_NAME (plain docker) - managed by start.sh" \
        --vpc-id "$VPC_ID" \
        --query 'GroupId' \
        --output text)"
...
echo "PathsGames stack started successfully (plain docker, image from Docker Hub)"
USERDATA
)"
...
# (We already confirmed it does NOT exist in the early short-circuit above.)
...
# Ensure instance carries the common tags (idempotent — also covers the reuse path)
aws ec2 create-tags \
    --region "$AWS_REGION" \
...
# When CloudFront is ON, ROUTE53_RECORD_NAME is an ALIAS to the distribution
# (created in the CloudFront block below), not a direct A record.
...
if [ "$ENABLE_CLOUDFRONT" = "true" ]; then
  if [ -z "$PUBLIC_DNS" ] || [ "$PUBLIC_DNS" = "None" ]; then
    ...
    # Idempotent: reuse a distribution already serving this hostname
...
CFEOF
        ...
        # Refresh the origin if the EC2 public DNS changed (e.g. instance recreated)
        CUR_CFG="$(mktemp)"
    ...
    # Point ROUTE53_RECORD_NAME at the distribution (Route53 alias A record)
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  Paths Games EC2 instance is RUNNING (plain docker)      ║"
echo "╠══════════════════════════════════════════════════════════╣"
printf "║  Instance ID : %-43s║\n" "$INSTANCE_ID"
printf "║  Name        : %-43s║\n" "$INSTANCE_NAME"
printf "║  Public IP   : %-43s║\n" "$PUBLIC_IP"
...
echo   "║  Redeploy: ./redeploy.sh   (pull latest test image)      ║"
echo   "║  Stop:     ./stop.sh                                     ║"
echo   "╚══════════════════════════════════════════════════════════╝"
