...
class StoryMysqlPersistenceRepository implements StoryPersistencePort
{
        ...
        foreach ($difficulties as $d) {
            ...
            if ($explicitId !== null) {
                ...
                $stmt->execute([
                    ...
                    ':trait_cost_positive_budget' => $d['traitCostPositiveBudget'] ?? null,
                    ':trait_cost_negative_budget' => $d['traitCostNegativeBudget'] ?? null,
                    ':life' => $d['life'] ?? 100,
                    ':energy' => $d['energy'] ?? 100,
                    ':sad' => $d['sad'] ?? 0,
    ...
    public function saveTraits(int $storyId, array $traits): void
    {
        ...
        foreach ($traits as $t) {
            ...
            $stmt->execute([
                ...
                ':energy' => $t['energy'] ?? 0,
                ':sad' => $t['sad'] ?? 0,
                ':dexterity' => $t['dexterity'] ?? 0,
                ':intelligence' => $t['intelligence'] ?? 0,
                ':constitution' => $t['constitution'] ?? 0,
                ':weight' => $t['weight'] ?? 0,
            ]);
        }
    }
    ...
    private function getTableColumns(string $tableName): array
    {
        ...
        $columnMap = [
            'list_stories_difficulty' => ['id', 'id_story', 'uuid', 'id_card', 'id_text_name', 'id_text_description', 'exp_cost', 'max_weight', 'min_character', 'max_character', 'cost_help_coma', 'cost_max_characteristics', 'number_max_free_action', 'trait_cost_positive_budget', 'trait_cost_negative_budget', 'life', 'energy', 'sad', 'dexterity', 'intelligence', 'constitution', 'weight'],
            ...
            'list_traits' => ['id', 'id_story', 'uuid', 'id_card', 'id_text_name', 'id_text_description', 'cost_positive', 'cost_negative', 'id_class_permitted', 'id_class_prohibited', 'life', 'energy', 'sad', 'dexterity', 'intelligence', 'constitution', 'weight'],
