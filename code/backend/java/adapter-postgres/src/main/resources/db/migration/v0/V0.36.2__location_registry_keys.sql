-- =============================================
-- Paths Games - Database Schema V0.36.2 (PostgreSQL)
...
COMMENT ON COLUMN list_locations.key_to_add IS 'Step 36.2 - registry key written when the party first arrives here; blank means no write';
COMMENT ON COLUMN list_locations.key_value_to_add IS 'Step 36.2 - the value written with key_to_add, through the same codec as an event effect';
COMMENT ON COLUMN list_locations.key_to_add_not_first IS 'Step 36.2 - registry key written on every arrival after the first; blank means no write';
COMMENT ON COLUMN list_locations.key_value_to_add_not_first IS 'Step 36.2 - the value written with key_to_add_not_first';
