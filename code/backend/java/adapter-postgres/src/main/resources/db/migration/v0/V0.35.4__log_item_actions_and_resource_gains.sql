-- =============================================
-- Paths Games - Database Schema V0.35.4 (PostgreSQL)
...
COMMENT ON COLUMN log_item_usage.action IS 'ADD | USE | DROP | REMOVE - what happened to the item; rows written before V0.35.4 are USE';
COMMENT ON COLUMN log_item_usage.id_event IS 'Event whose effect moved the item; NULL when the player used or dropped it directly';
COMMENT ON COLUMN log_item_usage.energy IS 'Signed energy delta the action produced; 0 on ADD and DROP';
COMMENT ON COLUMN log_item_usage.food IS 'Signed food delta the action produced; 0 on ADD and DROP';
COMMENT ON COLUMN log_item_usage.magic IS 'Signed magic delta the action produced; 0 on ADD and DROP';
COMMENT ON COLUMN log_item_usage.coin IS 'Signed coin delta the action produced; 0 on ADD and DROP';
COMMENT ON COLUMN log_events.energy_gain IS 'Energy the event gave the actor; the counterpart of log_events.energy';
COMMENT ON COLUMN log_events.food_gain IS 'Food the event gave the actor; the counterpart of log_events.food';
COMMENT ON COLUMN log_events.magic_gain IS 'Magic the event gave the actor; the counterpart of log_events.magic';
COMMENT ON COLUMN log_events.coin_gain IS 'Coins the event gave the actor; the counterpart of log_events.coin';
