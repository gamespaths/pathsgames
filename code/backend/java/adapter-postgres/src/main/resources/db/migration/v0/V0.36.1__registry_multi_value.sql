-- =============================================
-- Paths Games - Database Schema V0.36.1 (PostgreSQL)
...
-- would let every duplicate through here while SQLite blocked it. COALESCE is never NULL for a
-- value-bearing row, and an empty set is stored as NO row rather than as a null one.
...
COMMENT ON COLUMN list_keys.multi_value IS 'Step 36.1 - 1 = the key holds a set of values, each write adding a member; 0 = one value, each write replacing it';
COMMENT ON COLUMN gaming_state_registry.multi_value IS 'Step 36.1 - copied from list_keys when the key is first written in this match, and never reconciled: a running match keeps the behaviour it was born with';
