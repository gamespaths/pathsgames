...
-- Add explicit turn status column to gaming_turn_queue (PostgreSQL)
...
ALTER TABLE gaming_turn_queue ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'WAITING';

COMMENT ON COLUMN gaming_turn_queue.status IS 'Turn lifecycle: WAITING, ACTIVE, COMPLETED (Step 24)';
