-- =============================================
-- Paths Games - Database Schema V0.35.1 (PostgreSQL)
...
COMMENT ON COLUMN list_items.max_per_character IS 'Units one character may hold; 0/NULL = no limit. An ADD past it is refused, not an error';
COMMENT ON COLUMN list_items.amount_drop       IS 'Units removed by one drop-item; NULL = 1. Owning fewer drops what is there';
COMMENT ON COLUMN list_items.amount_use        IS 'Units consumed by one use-item; NULL = 1. Owning fewer refuses with ITEM_NOT_ENOUGH';
