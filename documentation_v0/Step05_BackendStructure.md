...
- **Schema Migration with Flyway**: The project uses **Flyway** for database schema versioning. Every schema change is a numbered SQL migration file committed to the repository. Flyway tracks applied migrations in a `flyway_schema_history` table and automatically executes new, unapplied migrations on application startup.
    - Each database adapter (`adapter-postgres`, `adapter-sqlite`) maintains its own dialect-specific SQL files under `src/main/resources/db/migration/`
    - **Versioned migrations** (`V{version}__{description}.sql`) are executed once and tracked by checksum
    - **Repeatable migrations** (`R__{description}.sql`) are re-executed when their content changes (used for dev/test seed data)
    - Version numbers are aligned with the project roadmap (e.g., Step 10 → `V0.10.x`, Step 12 → `V0.12.x`)
    - In **dev** profile: `validate-on-migrate=false`, `repair-on-migrate=true` for flexibility
    - In **prod** profile: `validate-on-migrate=true`, `repair-on-migrate=false` for strict checksums
