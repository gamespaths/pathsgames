...
- **Document Version**: 0.29.0 (here only due changes)
    ...
    | 0.29.0 | (still v0.29.0, no version bump) Movement availability verdict on `GET /api/match/{uuid}/info`: `locationsActive[].neighbors[]` gains `available`/`reason`, the exact twin — for movement — of the event verdict added above. New pure checker shared by `/info` and `POST /api/gameplay/{uuidMatch}/movements/start` so the two can never diverge (`MovementAvailabilityChecker.java`, `movement_availability.py`, AWS `lambda/match/movements.py`); check order matches the existing 8-code validation order (§3 of Step 28: `CHARACTER_CANNOT_ACT` → `MATCH_NOT_RUNNING` → `COMA` → `SLEEPING` → `NOT_A_NEIGHBOR` → `MOVEMENT_CONDITION_NOT_MET` → `OVERWEIGHT` → `INSUFFICIENT_ENERGY` → `LOCATION_FULL`); check context loaded once per request, no query inside the neighbor loop. OpenAPI `v0.19.0-match-creation-api.yaml` `LocationNeighborInfo` schema updated. No database/schema change. react-admin: Event Effects edit form gains `idTextName`/`idTextDescription` fields (order: `idCard`, `idTextName`, `idTextDescription`, `idEvent`, ...) and the entity list gains a `Name` column after the card — pure UI exposure of columns that already existed on the entity (`storiesEntities.jsx`); the existing generic form rules (saving name auto-fills description until picked manually; "new card" uses the name text) apply unchanged. react-game: mobile map rendering bugfix (no doc change — no design doc describes the map SVG rendering internals at that level of detail). See [Step28_MovementSystem.md — "Step 0.29.0 (addendum): Movement Availability Verdict on /info"](./Step28_MovementSystem.md). | July 13, 2026 |
    | 0.29.3 | Forced movement via event effects: `list_events_effects` gains a nullable `id_location` (Flyway `V0.29.3__event_effect_move_location.sql`, sqlite+postgres). When an executed effect row carries it, every recipient of that row (the usual `target`/`target_class` scope, INV-27) is MOVED to that location, **bypassing the entire Step 28 movement procedure**: no adjacency check, no energy cost, no availability verdict, no capacity check. Each actual move writes a cost-0 `log_movements` row (AWS: cost-0 entry in the match item's `movementLog`), so the Match Logs timeline still shows a `MOVEMENT` entry and the fog-of-war visited set stays truthful; a move to the recipient's current location is a no-op; an `id_location` matching no location of the story is authored noise and is silently skipped (resolved against a story location id→uuid map); later effects in the same chain resolve `target=ALL` at the recipient's NEW location (tracked per-character in the execution state). The execute-event response gains `movementApplied: boolean` and `locationChanges: [{characterUuid, fromLocationUuid, toLocationUuid}]` (both feed `refreshRecommended`); OpenAPI `v0.29.0-events-api.yaml` gains the `LocationChange` schema. Step 22 validator wires `event-effects.idLocation` into the existing `R_LOCATION_REF` dangling-reference check (Java `StoryValidatorService`, Python `story_validator_service.py`, AWS `story_validator.py`); `StoryImportService`/generic import and admin CRUD (`StoryCrudService.toMap`/`applyEventEffectFields`) carry the field as `idLocation`. Java: `EventEffectEntity`, `EventExecutionStorePort` (+`updateCharacterLocation`/`insertMovementLog`/`findLocationUuidsById`), `EventExecutionStoreAdapter`, `EventExecutionService.applyMovementEffect`, `ExecuteEventResponse`+`LocationChangeDto`. Python: `models.py`, `event_ports.py`, `event_store_adapter.py`, `event_service.py._apply_movement`, `event_models.py`+`LocationChange`, `event_controller.py`. AWS: `lambda/match/events.py.apply_location`, `lambda/match/handler.py` (location-uuid map, `movementApplied`, `locationChanges`), `lambda/seed/handler.py`. Seeds: sqlite/postgres dev seeds and Python `seed_dev_data.py` — story 9001 gains event 90028 (NORMAL, cost 2 energy, at the start location, card 90001) with effect 90023 (`target=ONLY_ONE`, `id_location=90006` Weather Observatory, deliberately not a neighbor of the start); AWS tutorial story gains location 3 "Hidden Grove" (`loc-tutorial-3`, no neighbor edge at all), event 28 `evt-step29-teleport` "Secret Passage" (cost 2 energy) and effect 14 (`idLocation: 3`) — the cost is 2 on purpose, so the Robot lookup "Event Uuid By Cost 1" keeps meaning the plain event. react-admin: event-effects form gains `idLocation` ("Move To Location ID (effect)", next to `idWeather`) in `storiesEntities.jsx`. react-game: **no change** — `GameBook` already reloads clock/weather/locations and the whole board after every executed event, so the new position arrives with the normal match-info reload. New Robot test "A Location Effect Teleports The Character Without Any Movement Check" in `29_events/events.robot` (17 → 18 tests), run on its own match (`New Teleport Match`, so the teleport can't strand the suite's shared character): finds the seed by behaviour (`Teleport Seed`), asserts the target is not a neighbor (`Neighbor Location Ids Of`), executes, asserts `movementApplied`/`locationChanges`/`energySpent==2`/new `currentLocationId`, and a cost-0 `MOVEMENT` log entry (`First Movement Log Entry`). Test counts: Java `mvn test -DskipITs` BUILD SUCCESS (752 unit tests: core 479, adapter-rest 163, adapter-admin 59, ms-launcher 32, adapter-sqlite 10, adapter-auth 9); Python 871 pass; AWS (unit) 510 pass; react-admin (vitest) 447 pass; full E2E Robot suite 458/458, zero regressions. See [Step29_NormalEvents.md — "Forced movement (v0.29.3)"](./Step29_NormalEvents.md) and [Step28_MovementSystem.md — "Step 0.29.3 (cross-reference)"](./Step28_MovementSystem.md). | July 17, 2026 |
