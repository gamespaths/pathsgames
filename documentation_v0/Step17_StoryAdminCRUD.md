...
For all backends (Java, Python, PHP), the system now enforces **composite primary keys** `(id, id_story)` for all sub-entities.
...
3. **MySQL (PHP)**: The schema uses `PRIMARY KEY (id, id_story)` with `id` as `AUTO_INCREMENT`. `setval` equivalent logic is applied during imports.
