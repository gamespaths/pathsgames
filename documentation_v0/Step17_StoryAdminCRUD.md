...
### 6.5 Scoped Numeric IDs on Create (SQLite + PostgreSQL)
For Step 17 create endpoints, if payload does not provide numeric `id`, backend now assigns the next scoped ID using story scope (`id_story`).

- Scope rule: uniqueness is guaranteed per story (`id`, `id_story`), not globally by `id` alone.
- Effect: fixes create failures previously seen as `NOT NULL` violations in SQLite for tables such as `list_choices` and `list_missions`.

### 6.6 PostgreSQL Mapping & Migration Compatibility
After adopting scoped keys, PostgreSQL required two compatibility updates:

1. **Flyway migrations (`V0.10.6`–`V0.10.9`)**
  - Removed FK constraints that referenced non-globally-unique `list_* .id` columns.
  - These columns remain typed as `BIGINT` and are resolved at application/domain level where needed.

2. **JPA mapping for `id_story`**
  - Shared `BaseStoryEntity.idStory` mapping is read-only by default.
  - Non-`@IdClass` entities override it as writable.
  - Prevents duplicate `id_story` insert bindings in PostgreSQL while keeping API payloads stable.
...
- `./run_robot_with_local_java_postgres.sh` → **178 passed, 0 failed**.
...
- **Document Version**: 0.17.3
    ...
    | 0.17.4 | Change primary keys on `list_` and `gaming_` tables get id values into import process | May 03, 2026 |
