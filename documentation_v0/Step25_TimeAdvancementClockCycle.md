...
- **Document Version**: 0.25.4
    ...
    | 0.25.2 | Per-player max stats (`lifeMax`/`energyMax`/`sadMax`/`weightMax`), carried weight, and item list added to all match-info endpoints, computed and persisted at character join across all backends. See §1. | June 15, 2026 |
    | 0.25.2 | Clock label fix: split `clockLabel` into `clockLabelSingular`/`clockLabelPlural` and fixed the AWS text-fallback resolution; `ClockWidget` uses the singular label as its tooltip. See §3 and §10.2. | June 16, 2026 |
    | 0.25.3 | Enriched `GET /api/match/{uuid}/info` with a new `locationsActive[]` block exposing each player-occupied location's card, neighbors, and events; `currentLocation*` now derives from the player's position. See §14. | June 16, 2026 |
    | 0.25.4 | Added a boolean `endGame` flag to `locationsActive[].events`; react-game renders an explicit end-game button when it is set. See §14.9. | June 16, 2026 |
