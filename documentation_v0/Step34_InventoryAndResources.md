...
- **Document Version**: 0.34.0
  ...
  |---------|-------------|------|
  | 0.34.0 | Inventory and resources, implemented. Four endpoints under `/api/gameplay/`: `GET .../inventory`, `POST .../inventory/use-item`, `POST .../inventory/drop-item`, `GET .../resources`. `use-item` answers the execute-event payload and deletes the whole inventory row (amount is not decremented); only `is_consumabile = 1` items can be used, gated by `id_class_permitted`/`id_class_prohibited`; `drop-item` applies neither gate. Effect application is delegated to a single new door on `EventExecutionService`, `applyStandaloneEffects`, so items go through the same Step-30 overflow/coma evaluator an event effect does — no second engine. `list_items_effects` gains `traits_to_add`/`traits_to_remove` (`V0.34.0__add_item_effect_traits.sql`, both dialects; postgres also converts `log_item_usage.effects_json` `JSONB`→`TEXT`). `effect_code` is case-insensitive with one alias, `SADNESS`→`sad` (`EffectStatCodec`). `/info players[].items[]` stays present for every player but is populated only for the calling user's character; `weight` is not masked; the admin view is exempt. `food`/`magic`/`coin` promoted onto the shared character stats block. Step 35: carried weight (`Σ item.weight × amount`) now feeds the movement gate, switching on the `OVERWEIGHT` refusal `MovementAvailabilityChecker` had always implemented against a hardcoded `0`; capacity stays the `weight_max` formula from steps 23/27. New `R_TRAIT_REF` validation rule (`Target.TRAIT`) checks trait ids named in effect CSVs. AWS un-hardcodes `_character_summary`/`_character_full`'s `weight`/`items` and adds four API Gateway routes. New robot suite `34_inventory` (25 tests). Test counts: coverage of new code 100% instructions on java/python/aws. | August 20, 2026 |
