...
| **StoryKey** | `list_keys` | Registry key definitions for a story: `id_card`, `id_story`, `name`, `value`, `id_text_description`, `group`, `priority`, `visibility`. `group`/`priority`/`visibility` were imported but never read anywhere until [Step36](./Step36_RegistrySystem.md), which joins them into the registry read as `category`/ordering/visibility. |
...
| **Item** | `list_items` | Item catalog: `id_card`, `id_story`, `id_text_name`, `id_text_description`, `weight`, `is_consumabile` (SQL column `INTEGER NOT NULL DEFAULT 1`, kept for a raw INSERT — every seed spells the value out; **v0.36.3**: an item that never declares the flag is now NOT consumable on every writer that goes through the model — Java's `ItemEntity.@PrePersist` and Python's `is_consumabile` column now default to `0`, and AWS's `is_consumable(item)` requires an explicit `1`. **Reverses v0.35.8**, which had unified the reading the other way — as consumable — after AWS alone read a missing key as a refusal; that reading let an item created from the admin console, whose checkbox is only sent once touched, become usable by accident), `id_class_permitted`, `id_class_prohibited`. **New in v0.35.0**: `flag_show_effects` (`INTEGER`, nullable, `DEFAULT 1`) — `1`/`NULL` = the inventory/`/info` promise (`effects[]`, see [Step35_ItemsResolution.md §6](./Step35_ItemsResolution.md#6-author-opt-out-flagshoweffects)) is reported; `0` = the item keeps its secret, `effects[]` comes back empty. Gates the promise only — `use-item` applies the row's effects identically either way. **New in v0.35.1** (see [Step35_ItemsResolution.md §8](./Step35_ItemsResolution.md#8-quantities-and-the-per-character-cap-v0351)), all three `INTEGER`, nullable: `max_per_character` — units of this item one character may hold; `0`/`NULL` = no limit, same reading as `id_class_permitted`/`id_class_prohibited`; an ADD past it is refused (reported, not an error). `amount_drop` — units removed by one `drop-item`; `NULL`/`≤0` reads as `1`; owning fewer is not a refusal, the drop takes what is there. `amount_use` — units consumed by one `use-item`; `NULL`/`≤0` reads as `1`; owning fewer **is** a refusal (`ITEM_NOT_ENOUGH`, 409). There is no `amount_add` — an event/choice ADD is always exactly one unit. |
...
| **StateRegistry** | `gaming_state_registry` | Key-value game registry for a match: `id_match`, `key`, `string_value`, `int_value`, `id_character`, `id_event`, `id_choice`, `clock`, `id_mission`, `id_mission_steps`. `(id_match, key)` is enforced **UNIQUE** since `V0.36.0` — [Step36](./Step36_RegistrySystem.md) made an assumption the upsert path had always made into a real constraint. |
...
| # | Invariant | Enforcement |
|---|-----------|-------------|
| **INV-33** | Registry keys are scoped to a single match (`id_match`); `(id_match, key)` is unique — at most one row per key per match | All registry operations include `id_match` in queries; `idx_state_reg_key` is a UNIQUE index since `V0.36.0` ([Step36](./Step36_RegistrySystem.md)) |
| **INV-34** | Registry values are either string or numeric (integer); booleans are numeric by convention — `0` is no, non-zero is yes (unified in v0.36.0; earlier content used string `"true"`/`"false"`) | `gaming_state_registry` has both `string_value` and `int_value` columns, never both set; `RegistryService.render`/`parse` are the single conversion (Step36) |
