...
15. Story content APIs — locations, neighbors, and cards
    ...
    - Implement location neighbor resolution service returning adjacency graph with directional edges (backend)
    - Implement card and image resolution service linked to locations, stories, and characters (backend)
    - Implement text resolution service supporting multi-language text lookup by id_text and lang (backend)
    - Build frontend API client for story locations with TypeScript interfaces and data mapping (frontend)
    - Write backend unit tests for location endpoints, neighbor service, card resolution, and text service (backend tests)
16. Story content APIs — events, choices, items, weather
    - Implement GET /api/v1/stories/{uuid}/events endpoint listing all events with types and effects (backend)
    - Implement GET /api/v1/stories/{uuid}/choices endpoint listing choices with conditions and effects (backend)
    - Implement GET /api/v1/stories/{uuid}/items endpoint listing items with effects and class restrictions (backend)
    - Implement GET /api/v1/stories/{uuid}/weather endpoint listing weather rules with probabilities and conditions (backend)
    - Implement GET /api/v1/stories/{uuid}/missions endpoint listing missions with steps (backend)
    - Build frontend API client for events, choices, items, weather, and missions with TypeScript types (frontend)
    - Write backend unit tests for all story content endpoints and data integrity validation (backend tests)
17. Story admin CRUD endpoints
    - Implement POST/PUT/DELETE /api/v1/admin/stories for story create, update, delete with admin role check (backend)
    - Implement admin CRUD for locations and neighbors under /api/v1/admin/stories/{uuid}/locations (backend)
    - Implement admin CRUD for events, event effects, choices, choice conditions, choice effects (backend)
    - Implement admin CRUD for items, item effects, weather rules, missions, mission steps (backend)
    - Implement admin CRUD for character templates, classes, class bonuses, traits (backend)
    - Implement admin CRUD for texts, cards, creators with multi-language support (backend)
...
20. Story validation and integrity checking
    ...
    - Implement GET /api/v1/admin/stories/{uuid}/validate endpoint returning validation report (backend)
...
21. Single-player match creation
    - Implement POST /api/v1/matches endpoint to create a single-player match with story and difficulty selection (backend)
    ...
    - Implement GET /api/v1/matches/{uuid} endpoint returning match details and status (backend)
...
22. Character template and class selection
    - Implement GET /api/v1/matches/{uuid}/characters/templates endpoint listing available character templates for the story (backend)
    - Implement GET /api/v1/matches/{uuid}/characters/classes endpoint listing available classes with bonuses (backend)
    - Implement POST /api/v1/matches/{uuid}/characters endpoint to select character template and class (backend)
...
23. Character traits and stats initialization
    - Implement GET /api/v1/matches/{uuid}/characters/traits endpoint listing available traits for selected class (backend)
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/traits endpoint to assign traits (backend)
...
25. Turn cycle engine for single-player
    ...
    - Implement POST /api/v1/matches/{uuid}/start endpoint to transition match from CREATED to RUNNING (backend)
    - Implement turn state machine: WAITING → ACTIVE → COMPLETED, tracking current turn in gaming_match (backend)
    - Implement POST /api/v1/matches/{uuid}/turns/pass endpoint for voluntary turn pass without energy cost (backend)
    - Implement GET /api/v1/matches/{uuid}/turns/current endpoint returning active turn details and timeout (backend)
...
26. Time advancement system — sleep, recovery, new time
    ...
    - Implement POST /api/v1/matches/{uuid}/turns/sleep endpoint for voluntary sleep action (backend)
...
27. Weather system — random selection and effects
    ...
    - Implement GET /api/v1/matches/{uuid}/weather endpoint returning current weather with movement cost modifiers (backend)
...
28. Movement system — adjacency, energy cost, validation
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/move endpoint accepting target location (backend)
    ...
    - Create log_movements record with from/to locations, energy cost, weather, and timestamp (backend)
...
30. Optional events — player-triggered actions
    - Implement GET /api/v1/matches/{uuid}/characters/{uuid}/events endpoint listing available optional events at current location (backend)
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/events/{uuid}/trigger endpoint to activate optional event (backend)
...
31. Choice engine — conditions, validation, presentation
    - Implement GET /api/v1/matches/{uuid}/characters/{uuid}/choices endpoint returning currently available choices (backend)
...
32. Choice resolution — apply effects and outcomes
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/choices/{uuid} endpoint to submit selected option (backend)
...
33. Inventory and item management
    - Implement GET /api/v1/matches/{uuid}/characters/{uuid}/inventory endpoint listing items with weight and effects (backend)
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/inventory/{uuid}/use endpoint to use consumable item (backend)
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/inventory/{uuid}/discard endpoint to discard item (backend)
...
34. Resource management — food, magic, coins, weight
    - Implement GET /api/v1/matches/{uuid}/characters/{uuid}/resources endpoint returning food, magic, coins, total weight (backend)
...
35. Registry system — key-value game state tracking
    - Implement GET /api/v1/matches/{uuid}/registry endpoint returning all visible registry entries grouped by category (backend)
...
36. Mission tracking and progression
    - Implement GET /api/v1/matches/{uuid}/missions endpoint listing active, completed, and available missions (backend)
    - Implement mission activation service triggered by registry value changes (condition_key from → to) (backend)
    - Implement mission step progression tracking within gaming_state_registry linked to list_missions_steps (backend)
...
37. Experience and character advancement
    ...
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/advance endpoint to spend experience on stat increase (backend)
...
38. Edge states — coma, sadness overflow, game over
    ...
    - Implement coma rescue: character in same location can spend energy to give COS life points to comatose character (backend)
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/rescue endpoint for coma rescue action (backend)
    ...
    - Write backend unit tests for sadness overflow, coma trigger, rescue mechanics, group coma, and game over transitions (backend tests)
39. Action logging and match history
    ...
    - Implement GET /api/v1/matches/{uuid}/history endpoint returning paginated action history with filters (backend)
...
40. Match snapshots — save and restore
    ...
    - Implement POST /api/v1/admin/matches/{uuid}/snapshots endpoint for manual full snapshot creation (backend)
    - Serialize full match state to JSONB: all gaming_* tables, registry, inventories, locations, turn queue (backend)
    - Implement POST /api/v1/admin/matches/{uuid}/snapshots/{uuid}/restore endpoint to restore state (backend)
    - Validate snapshot integrity against current schema version before restoration (backend)
    - Implement GET /api/v1/admin/matches/{uuid}/snapshots endpoint listing available snapshots with timestamps and types (backend)
...
43. User registration endpoint (email, username, password)
    - Implement POST /api/v1/auth/register endpoint accepting email, username, and password (backend)
...
44. Email verification flow and account activation
    - Implement GET /api/v1/auth/verify?token={token} endpoint to verify email and activate account (backend)
    ...
    - Implement POST /api/v1/auth/verify/resend endpoint to regenerate verification token and resend email (backend)
...
45. Google SSO integration
    ...
    - Implement POST /api/v1/auth/sso/google endpoint accepting authorization code from frontend (backend)
...
46. Credential login and JWT token management
    - Implement POST /api/v1/auth/login endpoint accepting username/email and password (backend)
    ...
    - Implement POST /api/v1/auth/refresh endpoint for token rotation with old refresh token invalidation (backend)
    - Implement POST /api/v1/auth/logout endpoint to revoke refresh token (backend)
...
47. User profile and account management
    - Implement GET /api/v1/users/me endpoint returning user profile: username, email, nickname, theme, stats (backend)
    - Implement PUT /api/v1/users/me endpoint to update display name, nickname, theme, and language (backend)
    - Implement POST /api/v1/users/me/password endpoint for password change with current password verification (backend)
    - Implement DELETE /api/v1/users/me endpoint for account deletion with confirmation and data cleanup (backend)
    - Maintain user statistics: total matches played, completed, abandoned, total play time (backend)
    - Build frontend profile page with edit forms, password change dialog, and account deletion confirmation (frontend)
    - Write backend and frontend unit tests for profile CRUD, password change validation, deletion cascade, and stats tracking (tests)
48. Link guest sessions to registered accounts
    - Implement POST /api/v1/users/me/link-guest endpoint accepting guest cookie token to merge sessions (backend)
...
54. WebSocket lifecycle logging and monitoring
    ...
    - Expose WebSocket health endpoint: GET /api/v1/admin/websocket/status with connection statistics (backend)
...
55. Multiplayer match creation endpoint
    - Implement POST /api/v1/matches endpoint supporting multiplayer match creation with min/max player count (backend)
...
56. Match lobby and player joining
    - Implement GET /api/v1/matches?status=CREATED endpoint listing joinable matches with filters (backend)
    - Implement POST /api/v1/matches/{uuid}/join endpoint for player to join an existing match lobby (backend)
    - Validate: match accepting players, not full, player not already in another active match (if restricted) (backend)
    - Create gaming_character_instance placeholder with id_user linked and status JOINING (backend)
...
57. Multiplayer character selection in lobby
    ...
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/ready endpoint to lock character selection (backend)
...
58. Lobby state sync and readiness tracking
    - Implement GET /api/v1/matches/{uuid}/lobby endpoint returning full lobby state: players, characters, readiness (backend)
    ...
    - Implement POST /api/v1/matches/{uuid}/leave endpoint for players to leave lobby before match start (backend)
...
59. Match start — validate players and initialize state
    - Implement POST /api/v1/matches/{uuid}/start endpoint (creator-only or auto-trigger) for multiplayer start (backend)
...
68. Group movement — follow invitations
    - Implement POST /api/v1/matches/{uuid}/characters/{uuid}/move/invite endpoint to send follow invitation (backend)
    ...
    - Implement POST /api/v1/matches/{uuid}/movements/invites/{uuid}/accept endpoint for accepting follow (backend)
    ...
    - Write backend and frontend unit tests for invite creation, notification, accept/decline flow, timeout, and free movement (tests)
...
71. Trade system — propose, accept, reject
    - Implement POST /api/v1/matches/{uuid}/trades endpoint to create trade proposal with items and resources offered/requested (backend)
    - Implement GET /api/v1/matches/{uuid}/trades/{uuid} endpoint to view trade proposal details (backend)
    - Implement POST /api/v1/matches/{uuid}/trades/{uuid}/accept endpoint for recipient to accept trade (backend)
    - Implement POST /api/v1/matches/{uuid}/trades/{uuid}/reject endpoint for recipient to reject trade (backend)
    ...
    - Write backend unit tests for trade creation, view, accept/reject, location validation, and WebSocket notification (backend tests)
...
73. In-game chat system
    - Implement POST /api/v1/matches/{uuid}/chat endpoint to send chat message (backend)
    - Implement GET /api/v1/matches/{uuid}/chat endpoint returning paginated chat history (backend)
...
74. Chat moderation, filtering, and rate limiting
    ...
    - Implement admin mute capability: POST /api/v1/admin/matches/{uuid}/users/{uuid}/mute (backend)
...
75. Notification queue and priority system
    ...
    - Implement GET /api/v1/matches/{uuid}/notifications endpoint returning unread notifications (backend)
    - Implement POST /api/v1/matches/{uuid}/notifications/{uuid}/read endpoint to mark as read (backend)
    - Build frontend notification center with badge count, priority sorting, and toast popups for critical items (frontend)
    - Write backend and frontend unit tests for notification queue, priority sorting, delivery, read marking, and UI (tests)
...
77. Admin dashboard — match overview and player info
    - Implement GET /api/v1/admin/matches endpoint listing all matches with status, player count, and timestamps (backend)
    - Implement GET /api/v1/admin/matches/{uuid}/details endpoint returning full match state, characters, and registry (backend)
    - Implement GET /api/v1/admin/matches/{uuid}/players endpoint listing connected players with session info (backend)
    - Implement GET /api/v1/admin/matches/{uuid}/logs endpoint returning filtered action logs (backend)
    - Add admin authorization check (ADMIN role) to all /api/v1/admin/** endpoints (backend)
...
78. Admin actions — force turn, end match, restore snapshot
    - Implement POST /api/v1/admin/matches/{uuid}/force-turn endpoint to advance turn on stuck matches (backend)
    - Implement POST /api/v1/admin/matches/{uuid}/end endpoint to forcefully terminate a match (backend)
    - Implement POST /api/v1/admin/matches/{uuid}/pause and /resume endpoints for match state control (backend)
    - Implement POST /api/v1/admin/matches/{uuid}/registry endpoint to edit registry values directly (backend)
...
79. Admin user management — suspend, ban, unlock
    - Implement POST /api/v1/admin/users/{uuid}/ban endpoint to ban user (state=4, remove from active matches) (backend)
    - Implement POST /api/v1/admin/users/{uuid}/suspend endpoint to temporarily block user (state=3) (backend)
    - Implement POST /api/v1/admin/users/{uuid}/unblock endpoint to restore user to active state (backend)
    - Implement GET /api/v1/admin/users endpoint listing users with search, filter by state, and pagination (backend)
    - Implement POST /api/v1/admin/users/{uuid}/kick endpoint to remove user from specific match (backend)
