...
| `Step32_ChoiceResolution.md` | 16 | Choice resolution: `POST .../action/select-choice` applies `list_choices_effects` (stats, registry, items, forced movement, weather, inline events via new v0.32.0 columns), runs `id_event_torun`, reveals the withheld narrative, records `log_choices_executed` + `gaming_story_progress`, writes `CHOICE_SELECTED`; charges nothing — open-cycle cost-bypass guard; `V0.32.0__choice_effect_targets.sql` | choice resolution, select-choice, list_choices_effects, CHOICE_SELECTED, flag_group, is_progress |
