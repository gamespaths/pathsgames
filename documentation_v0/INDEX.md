...
| `Step38_ExperienceSystem.md` | `gaming_character_instance.exp`, written since Step 29, finally gets a spender: `use-exp` raises DEX/INT/COS by one at a difficulty-priced cost. `is_safe` dropped for `secure_param`, `cost_max_characteristics` replaced by `exp_cost_base`/`max_stat_value`. | use-exp, exp_cost_base, max_stat_value, EXP_USE, ExperienceCard, secure_param, is_safe |
