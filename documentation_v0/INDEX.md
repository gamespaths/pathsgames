...
| `Step37_MissionSystem.md` | Missions are a projection of the Step 36 registry: no operator, no state table, `condition_value`/`condition_values` (PIPE AND) drive an `AVAILABLE`→`ACTIVE`→`COMPLETED`/`FAILED` machine. New `/missions` endpoints, `missions[]` on `/info`, react-game bookmark live. **v0.37.1**: start-location mission trigger bugfix, admin Missions tab, react-game mission steps view. | missions, MissionService, condition_values, MissionsCard, MissionStepsCards, openMission, AVAILABLE, FAILED |
