...
| `Step12_GuestLoginMethod.md` | Guest login, JWT token structure. v0.36.2: admin guest list paged; **v0.37.5**: AWS cookie-token resume reads GSI2Summary (GSI1 fallback). | guest, login, JWT, admin-guests, pagination, stale-purge, GSI2Summary |
...
| `Step15_StoryContentAPIs.md` | Story content APIs: categories and groups. **v0.37.5**: AWS listing reads GSI2Summary, not raw story item. | categories, groups, GSI2Summary |
...
| `Step17_StoryAdminCRUD.md` | Admin CRUD for all story entities; **v0.35.8**: texts capped at 2000 chars. **v0.37.5**: AWS `POST /api/admin/cache/flush`, story cache invalidation. | admin, CRUD, textLimits, cache flush, story cache, GSI2Summary |
...
| `Step19_SinglePlayerMatchCreation.md` | Single-player match creation. **§6.1 (v0.32.1)**: one active match per user/story, 409 `ACTIVE_MATCH_ALREADY_EXISTS`; **v0.37.4**: GSI1Summary perf fix; **v0.37.5**: GSI2Summary admin/story/guest indexes | match, create, ACTIVE_MATCH_ALREADY_EXISTS, RESUME_WITHOUT_MODAL, GSI1Summary, GSI2Summary |
