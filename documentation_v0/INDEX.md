...
| `Step09_DesignCoreDataModel.md` | **Core data model.** Entities, relationships, persistent vs transient, valid game states, invariants; **v0.36.3**: `is_consumabile`'s default flips to non-consumable on all three backends | entities, data model, invariants, game state, short_text, is_consumabile |
...
| `Step12_GuestLoginMethod.md` | Guest login, JWT token structure. v0.36.2: admin guest list now paged (`{items,nextCursor,limit}`), new stale-purge preview/delete endpoints. | guest, login, JWT, admin-guests, pagination, stale-purge |
| `Step13_SessionTokenManagement.md` | Session/token management, auth filter | session, token, auth filter |
| `Step14_StoriesImportSystem.md` | Story import system and data seeding; **v0.36.3**: Java's import now maps the v0.32.0 choice-effect targets it had dropped since Step 32 | import, seed, stories, linkDeferredReferences, link_deferred_references, deferred references, locationNeighbors, align_schema, cascading delete |
...
| `Step18_GameMainFrontend.md` | react-game frontend: structure, design system, API client + mock fallback, guest identity; **v0.35.8**: `StoryCard.jsx` over shared `Card`, `matchesStatus`-gated footer. **v0.36.2**: GAMEOVER counts as completed, StoryCard shows Replay button. | react-game, frontend, design system, GameBook, useBookView, StoryCard, Replay, FINISHED_MATCH_STATUSES |
...
| `Step29_NormalEvents.md` | Normal (player-triggered) events: schema V0.29.0, check procedure, execution, `available` flag, logs; **v0.36.3**: AWS forced-move/time-end race and `automaticEvents[]` on `execute-event` fixed | events, execute-event, effects |
...
| `Step33_LocationEntryEvents.md` | Location entry events: triggers bind on five pre-existing `list_locations` columns; counter-zero finally executed (Step 26's dead end closed); `flag_visited` party-scoped visited state; **v0.36.3**: AWS/Python `execute-event` now answers `automaticEvents[]` | location entry, automatic trigger, counter zero, flag_visited, priority_automatic_event, cardEffects, cardLocation, AutomaticEvents |
...
| `Step36_RegistrySystem.md` | Registry becomes a system: one `RegistryService` (render/parse/evaluate) replaces eight readers and three writers, compared via ∃/∄/∀ operators (v0.36.1). v0.36.2: case-insensitive compare, location registry writes, admin registry edit API. **v0.36.3**: admin `/info` now includes hidden keys. | registry, RegistryService, evaluate, admin-edit, case-insensitive, multi_value, includeHidden, REGISTRY_CHANGE |
