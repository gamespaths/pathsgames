...
| `Step37_MissionSystem.md` | Missions are a projection of the Step 36 registry: no operator, no state table, `condition_value`/`condition_values` (PIPE AND) drive an `AVAILABLE`→`ACTIVE`→`COMPLETED`/`FAILED` machine. New `/missions` endpoints, `missions[]` on `/info`, react-game bookmark goes live. | missions, MissionService, condition_values, R10_MISSION_CONDITION, stepReached, AVAILABLE, FAILED |
