...
### 14.1 v0.37.1 bugfix — the start location's own pair never wrote

Because the party is seeded `flag_visited = 1` on the starting location at match creation
([Step 33](./Step33_LocationEntryEvents.md)) and `writeArrivalRegistry` only runs inside
`resolveArrival`, which only `onArrival` calls, the start location's `key_to_add` /
`key_value_to_add` pair could never write in any match, ever — the one authorable field this
step introduced with no path to fire. New `RegistryService.writeStartLocationEntry(idMatch,
idCharacter, clock)` runs once, at the CREATED → RUNNING transition (`TurnCycleService.startMatch`,
right after `weatherService.applyAtTimeStart`), where the clock and the active character both
already exist; it goes through the same `upsert`, so a mission gated on that key opens
immediately. `flag_visited` and `key_to_add_not_first` are untouched. Mirrored on Python
(`registry_service.write_start_location_entry`, wired into `turn_cycle_service.start_match`)
and AWS (`_write_start_location_registry`, called from `_start_match` before the match is
persisted). See [Step37 §2](./Step37_MissionSystem.md#2-state-storage) for why this mattered
...
| Engine (AWS) | `lambda/match/registry.py` (new); `lambda/match/events.py` (`apply_registry` now delegates, bug §9.1); `lambda/match/handler.py` (`_edge_condition_met` consolidation — bug §9.2 —, `_weather_condition_matches`, `_get_match_registry`, dispatcher route). **36.1**: `registry.py` gains `rows_in`/`values_in`/`remove`; `upsert(..., story=None)` (§10). **36.2**: `registry.py` gains `_norm`/`_eq` (§13); `handler.py _resolve_arrival` gains `_write_arrival_registry` (§14); new `PUT`/`DELETE` registry routes and admin-weather registry fields (§15, [Step27](./Step27_WeatherSystem.md)). **36.3**: `handler.py _detail_from_item` passes `all_locations` into `_registry.list_entries` (§8.1); new `_reread_characters` and `db_utils` `ConsistentRead=True` (see [Step29 §3](./Step29_NormalEvents.md#3-execution)). **37.1**: `handler.py _write_start_location_registry(match, story, characters, top)`, called from `_start_match` before the match item is persisted (§14.1). **37.5** (round 2): the `36.3` `_reread_characters`/`ConsistentRead=True` pair is superseded by the per-request `repo.py` unit of work ([Step19 §6](./Step19_SinglePlayerMatchCreation.md), [Step29 note](./Step29_NormalEvents.md#3-execution)); registry GET routes now read `consistent=False` |
...
- **Document Version**: 0.37.4
  ...
  | 0.37.5 | AWS-only cost pass, round 2, no behaviour change: `_reread_characters`/`ConsistentRead=True` (v0.36.3) superseded by the per-request `repo.py` unit of work; registry GET routes read `consistent=False`. See Engine (AWS) row above. | September 15, 2026 |
