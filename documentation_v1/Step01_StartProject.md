...
6. Tables  (all table must have timestamp column `ts_insert` and `ts_update` )
	...
	2. `users` (id, username, password_hash, email_address, google_id_sso, rule (ADMIN/PLAYER), ts_registration, ts_last_access, nickname, state (1=registration, 2=active, 3=blocked, 4=banned, 5=password), language, 
	3. `users_tokens` (id, id_user, refresh_token, expires_at, revoked)
	4. `list_stories` (id, id_card, id_text_title, id_text_description, author, version_min, version_max, id_location_start, id_image. id_location_all_player_coma, id_event_all_player_coma, clock_singular_description (hour), clock_plural_description (hours), id_event_end_game, id_text_copyright, link_copyright, id_creator)
	...
	6. `list_keys` (id, id_card, id_story, name, value, id_text_description)
	...
	24. `list_cards`  (id, id_card, lang, url_immage, id_text_title, id_text_description, id_text_copyright, link_copyright, id_creator, alternative_image, awesome-icon, style_main, style_detail )
	25. `list_texts` (id, id_story, label, lang, text, short_text, id_text_copyright, link_copyright, id_creator)
	26. `list_creator` (id, lang, id_text, link, url, url_image , url_emote, url_instagram)
	...
	31. `gaming_inventory_items` (id, id_character_match, id_item, amount)
	32. `gaming_state_registry` (id, id_match, key, string_value, int_value )
...
- ipotesi Rest-API (Non mostrare tutti i luoghi vicini se non sono mai stati visitati)
	...
	- storie e partite (stories e games) 
		...
		- GET /api/stories/{id}/characters: Elenco dei characters selezionabili per quella storia.
		...
		- POST /api/games/{id}/leave: Se la partita non è ancora iniziata (CREATA): rimuove il character. Se è IN_CORSO: marca il character come ABBANDONATO
		...
		- POST /api/games/{id}/select-character: Scelta del character prima dell'inizio.
	...
	- stato del gioco gioco (game)
		...
		- GET /api/game/{id}/players/{playerId}/stats: dettaglio di tutte le statistiche di un character comprese caratterstiche, stati, effetti, inventory, ... 
		- GET /api/game/{id}/characters/{id}: Dettagli granulari di un character + statistiche.
		...
		- GET /api/game/{id}/turnSequence: Ritorna la coda turni completa (gioco_coda_turni) con ordine e stato (attivo/addormentato). Utile per il frontend per mostrare "Chi agisce dopo di me?".
	...
	- azioni di gioco (gameplay) 
		- POST /api/gameplay/{id_game}/movements/start: Sposta il character (parametri: idLuogoDestinazione). *(1)
		- GET /api/gameplay/{id_game}/movements/pending: Lista degli inviti di movimento di gruppo in sospeso per tutti i characters della partita
		- POST /api/gameplay/{id_game}/movements/confirm-movement-invite: api usata da un character per seguire un altro character che si è mosso (parametri: idcharacterDaSeguire)
		- POST /api/gameplay/{id_game}/action/ask-help: Chiedi aiuto a un altro character nello stesso luogo (parametri: idcharacterAiutante) *(1) -> manda messaggio di aiuto
		- POST /api/gameplay/{id_game}/action/help-player: api usata da un character per aiutare un altro (se si trova nello stesso luogo e se ha abbastanza energia)
		...
		- POST /api/gameplay/{id_game}/action/sleep: Il character decide di addormentarsi volontariamente *(1) questo cambia il id_character_current_turn
		- POST /api/gameplay/{id_game}/action/pass: (tipo ent-turn) Passa il turno senza fare nulla. con verifica id_character_current_turn, questo cambia il id_character_current_turn e aumenta/verifica il counter_consecutive_pass, se counter_consecutive_pass>PARAMETRO -> gameover
		- GET  /api/gameplay/{id_game}/inventory: Contenuto dello inventory e peso attuale e il peso attuale (di tutti i characters)
	...
	- admin (admin)
		...
		- POST /api/admin/game/{id_game}/register chiama chiaveRegistroAggiungi (id_match, chiave, valore)
	...
	- tutte quelle con *(1) = con verifica id_character_current_turn e eventualmente aggiorno counter_consecutive_pass
- WebSocket "coda rigorosa" e azioni che influenzano tutti, le API REST da sole non bastano. WebSocket per notificare gli eventi in tempo reale:
	- Topic: /topic/game/{id_game}
		...
		2. TURN_UPDATE: Questo messaggio deve partire ad ogni cambio di turno o cambio ordine (pass). (ex TURN_LOCKED e TURN_FREE: L'azione è finita, la coda è libera.)
			   ```json
			   {
				 ...
				 "queueSequence": [101, 102, 103], // Ordine visivo per il client
				 "consecutivePasses": 2, // Se arriva a N -> penalità
				 "isStalemateWarning": true // Avviso grafico se rischio stallo
			   }```
		3. GAME_EVENT: "Piove!", "Il Giocatore B ha attivato una trappola: tutti perdono 1 Vita". Questa ricausa nel frontend una refresh di characters/luoghi/eventi/...
		4. DAY_END: Tutti i characters hanno dormito, inizia un nuovo tempo
	...
	- prevedere un Timeout del Lock lato server. Se giocatore non completa una azione entro X secondi, il server rilascia il lock forzatamente e "l'evento termina", energia spesa non rimborsata, registro modificato rimane così e il character di addormenta in automatico!
...
- **Document Version**: 1.2
	...
	- 1.2: traslated and updated roles and table section (March 13, 2026)
